...
CREATE TABLE if not exists ims.orderLine(order_id int not null, item_id int not null,quantity int default 1,FOREIGN KEY (order_id) REFERENCES orders (order_id) on delete cascade,FOREIGN KEY (item_id) REFERENCES items (id) on delete cascade);
